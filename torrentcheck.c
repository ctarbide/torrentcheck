...
int
beParseInteger(BYTE *benstr, int benstrLen, int benstrOffset, INT64 *longOut)
{
	INT64 foundValue = 0;
	INT64 negPos = 1;
	int i;
...
int
beParseString(BYTE *benstr, int benstrLen, int benstrOffset, BYTE **stringBegin, int *stringLen)
{
	int i;
	BYTE b;
	int foundLen = 0;
...
int
beFindInDict(BYTE *benstr, int benstrLen, int benstrOffset, BYTE *dictKey)
{
	BYTE *stringPtr;
	int stringLen;
	int dictKeyLen;
...
int
beStepOver(BYTE *benstr, int benstrLen, int benstrOffset)
{
	BYTE *bp;
	int ip;
...
char *
print64(INT64 val, char *buf, char useCommaDot)
{
...
void
backspaceProgressLine(int *showProgressChars)   // remove the progress line by backspacing
{
...
sha1Filter(char *compareHash)
{
	...
	if (strcasecmp(hexHash, compareHash) == 0) {
		return 0;
	}
...
main(int argc, char *argv[])
{
	...
	if (memcmp(sha1hash, "\xac\x1f\xd0\xda\xea\x37\x65\x87\x9a\xde\xfa\x33\x38\x62\x71\xf3\x85\x08\xa8\xbd", SHA1_LEN) != 0) {
		errorsFound++;
	}
	...
	if (ofs >= 0) { // single file
		...
		if ((thisFileOffset >= 0) && (thisFileOffset < torrentLen - 1) &&
			(torrent[thisFileOffset] == 'l')) {
			thisFileOffset++;
		}

		while ((thisFileOffset >= 0) && (thisFileOffset < torrentLen)) {
			if (torrent[thisFileOffset] != 'd') {
				break;
			}
	...
	if ((multiFileTorrent) && (contentPath != NULL)) { // multi-file torrent check
		...
		while (currentFile < numFiles) {
			...
			while (fileBytesRead < fileBytesExpected) {
				...
				if (fp != NULL) {
					readLen = fread(pieceBuf + bytesRead, 1, bytesToRead, fp);
				} else {
					readLen = 0;
				}
